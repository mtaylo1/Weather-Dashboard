...
var getCurrentWeather = (event) => {
    ...
    fetch(URLfetch)
        ...
        .then((response) => {
            ...
            let currentMoment = new Date().toDateString()
            let currentWeatherBox = "https://openweathermap.org/img/w/" + response.list[0].weather[0].icon + ".png";
        ...
        <h3>${response.list[0].name} ${currentMoment}<img src="${currentWeatherBox}"></h3>
        <ul class="list-unstyled">
            <li>Temperature: ${response.list[0].main.temp}&#8457;</li>
            ...
            let latitude = response.list[0].coord.lat;
            let longitude = response.list[0].coord.lon;
            let URLfetch = "http://api.openweathermap.org/data/2.5/uvi?lat=" + latitude + "&lon=" + longitude + "&APPID=" + APIkey;
    ...
    fetch(URLfetch)
        ...
        .then((response) => {
            ...
            for (let i = 0; i < response.list.length; i++) {
                ...
                let rightNow = moment.unix(dayTimeUTC).utc().utcOffset(timeZoneOffsetHours);
